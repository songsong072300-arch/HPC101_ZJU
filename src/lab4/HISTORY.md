...
1. 将 `Final_Evolution_Time` 恢复为CPU规定值 `40.0`，关闭 TwoPuncture cache，用O8推荐的 owner-local 16线程配置完成至少一次长跑。
2. 对正式长跑执行 `./check.sh` 和关键输出比较，确认长时间轨迹、constraints与波形。
3. 再重复一次未插桩短跑；unbound调度以中位数报告，并继续寻找更稳定的亲和性方案。
4. 为固定表面点预计算并缓存插值stencil索引和权重，避免每个分析时刻、每个变量重复执行 `polin3/polint`。
5. 缓存优化后只采样owner rank或降低采样扰动；若分析等待消失，再转向 `compute_rhs_bssn_` 的行级优化。
