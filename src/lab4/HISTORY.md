...
### R88：NRELAX 扫描（250/275/300/325）

状态：**保留 NRELAX=250（饱和点）**。

日期：2026-08-22。

计算节点 30 线程独立运行 TwoPunctureABE（每个值 3 轮）：

| NRELAX | BiCGStab 迭代 | Newton |F| | 裸质量 |
|--------|---:|---|---|
| 250 | 86 | 2.518e-13 | 0.576976/0.378578 |
| 275 | 86 | 2.518e-13 | 完全一致 |
| 300 | 86 | 2.518e-13 | 完全一致 |
| 325 | 86 | 2.518e-13 | 完全一致 |

NRELAX≥250 后 BiCGStab 迭代数不再减少（预条件强度饱和）。继续增大只增加
单次 relax 工作量（250→325 = +30% sweeps）不减少迭代数。**NRELAX=250 是
最优值**。
...
计算节点 t=10 A/B（O78+O79+O88，owner-local 16 线程，无 cache）：

| 配置 | Total Evolve 中位数 | Program Cost 中位数 | vs 30×2 |
|------|---:|---:|---|
| 30×2 (current) | 124.7 s | 154.3 s | — |
| 30×1 | 127.9 s | 156.3 s | Evolve +2.5% |
| 20×3 | 235.1 s | 264.5 s | Evolve +88.6% |
